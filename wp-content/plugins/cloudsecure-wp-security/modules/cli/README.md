...
```bash
wp cldsec-wp-security enable two-factor-authentication --enabled_roles=administrator,editor --xmlrpc_login=1
```
...
| --xmlrpc_login     | XML-RPC経由のログイン制限          | 0(許可), 1(拒否)                                           |

**注意事項:**
- `xmlrpc_login` のデフォルト値は `1`（拒否）です。2段階認証が有効な権限グループのユーザーに対して、XML-RPC経由のログインをすべて拒否します。
